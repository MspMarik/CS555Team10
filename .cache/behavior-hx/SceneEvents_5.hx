...
class SceneEvents_5 extends SceneScript
{
	...
	override public function init()
	{
		...
		addActorEntersRegionListener(getRegion(0), function(a:Actor, list:Array<Dynamic>):Void
		{
			...
			{
				switchScene(GameModel.get().scenes.get(0).getID(), null, createCrossfadeTransition(0.2));
			}
		});
		...
		addActorEntersRegionListener(getRegion(1), function(a:Actor, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled && sameAsAny(getActorType(2),a.getType(),a.getGroup()))
			{
				switchScene(GameModel.get().scenes.get(6).getID(), null, createCrossfadeTransition(0.2));
			}
		});
